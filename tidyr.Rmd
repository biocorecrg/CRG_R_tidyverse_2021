...
Here we introduce some useful functions from the `tidyr` package to clean up and organize data so that to obtain **tidy** data that can then be processed more easily:

### Separate & Unite

Let's work on this **untidy** tibble `table5` that is a built-in data set from the `tidyr` package *(table5 displays the number of tuberculosis cases documented by the WHO in selected countries)*:
...
* Is there a problem with `patients3`?

Columns `height` and `weight` are **treated as characters** (because they contained characters "kg" and "cm").
<br> 
We can convert them into numbers with the `mutate_at` function from the `dplyr` package:
