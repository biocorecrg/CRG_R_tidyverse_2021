...
```{r, eval=F}
group_by(starwars, species, hair_color, gender) %>%
      summarise(n())
```
...
```{r, eval=TRUE}
sw_sg <- group_by(starwars, species, gender) 
```

We can then use `slice_max` (function that retrieves the entry that contains the maximum value in the specified variable) to retrieve the character with the **maximum `height`**:
 
```{r, eval=TRUE}
sw_sg %>% 
  select(name, species, gender, height) %>% # we are selecting the columns here just to make the output more readable
  slice_max(height)
```
...
<br>
As the data is grouped by `species`, we will get the value **per species**, not taking into account the whole tibble:
...
<br>
While this is not an issue when you are summarizing (you get a small summary table), it can be useful in case you are using the grouping - for example - to **create a new column** (example stated above):
...
If now you want to add a column that describes the **average mass of ALL individuals**, you need to **ungroup** the tibble:

```{r}
# Without ungrouping, you get the average mass calculated per species
starwars %>% group_by(species) %>% 
  select(name, species, height, mass) %>% 
  mutate(average_height_species=mean(height, na.rm=TRUE)) %>%
  mutate(average_mass=mean(mass, na.rm=TRUE))
...
Extract the row that has the maximum "height" from in the `starwars` dataset:

```{r}
slice_max(starwars, order_by=height)
...
```{r}
slice_min(starwars, order_by=mass,
          n=2)
```

Check the first or last row with `slice_head` and `slice_tail`, respectively
...
slice_head(starwars, prop=0.1)

# last row
slice_tail(starwars)

# last 7 rows
slice_tail(starwars, n=7)

# last 25% of the rows
slice_tail(starwars, prop=0.25)
```
...
# 15% of rows selected randomly
slice_sample(starwars, prop=0.15)
```
...
relocate(presidential, party)

# move column "name" at the end (after last column)
relocate(presidential, name, .after=last_col())

# move column "name" before column "party"
relocate(presidential, party, .before=party)

# move around all columns
relocate(presidential, party, start, name, end)

# rename a column as you relocate it
relocate(presidential, President=name, .after=last_col())
```
